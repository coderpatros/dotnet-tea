...
internal static class GetComponentReleasesCommand
{
    public static Command Create(GlobalOptions globals)
    {
        var uuidArg = new Argument<string>("uuid") { Description = "Component UUID" };

        var command = new Command("get-component-releases") { Description = "Get releases for a component" };
        ...
        command.SetAction(async (parseResult, ct) =>
        {
            ...
            var result = await client.GetComponentReleasesAsync(parseResult.GetValue(uuidArg)!, ct);
            formatter.WriteComponentReleases(result);
        });
